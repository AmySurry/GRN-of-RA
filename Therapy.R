...
therapy_levels <- c(-0.75,-0.5,-0.25, 0, 0.25, 0.5, 0.75, 1.0)
...
for (therapy_factor in therapy_levels) {
  ...
  parameters_therapy["k1"] <- parameters["k1"] * (1 + therapy_factor)  # Increase IKKa degradation
...
combined_results$Therapy <- factor(combined_results$Therapy,
                                   levels = c("100%", "75%", "50%", "25%", "0%", "-25%", "-50%", "-75%"))
...
grid.arrange(
  ...
  top = textGrob("Effect of regulating k1", gp = gpar(fontsize = 16, fontface = "bold"), hjust = 0, x=0.02)
  )
...
grid.arrange(
  ...
  nrow = 2,
  heights = c(10, 1),  # Adjust the relative heights (10 for plots, 1 for legend)
  top = textGrob("Effect of regulating a3", gp = gpar(fontsize = 16, fontface = "bold"), hjust = 0, x=0.02)
)
...
grid.arrange(
  ...
  nrow = 2,
  heights = c(10, 1),  # Adjust the relative heights (10 for plots, 1 for legend)
  top = textGrob("Effect of regulating i1a", gp = gpar(fontsize = 16, fontface = "bold"), hjust = 0, x=0.02)
)
